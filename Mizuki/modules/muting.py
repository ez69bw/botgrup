...
def mute(update: Update, context: CallbackContext) -> str:
    ...
    if member.can_send_messages is None or member.can_send_messages:
        ...
        bot.sendMessage(
            chat.id,
            f"Mute <b>{mention_html(member.user.id, html.escape(member.user.first_name))}</b> berhasil!",
...
def unmute(update: Update, context: CallbackContext) -> str:
    ...
    if member.status != "kicked" and member.status != "left":
        if (
            ...
            bot.sendMessage(
                chat.id,
                f"Oke unmute <b>{mention_html(member.user.id, html.escape(member.user.first_name))}</b> kasian jomblo!",
                parse_mode=ParseMode.HTML,
            )
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
...
@bot_admin
@can_restrict
def muteme(bot: Bot, update: Update, context: CallbackContext) -> str:
...
MUTEME_HANDLER = CommandHandler(
    ["muteme", "mauadmin"], muteme, pass_args=True, admin_ok=True
)
...
__mod_name__ = "々 Muting 々"
__handlers__ = [MUTE_HANDLER, UNMUTE_HANDLER, TEMPMUTE_HANDLER, MUTEME_HANDLER]
